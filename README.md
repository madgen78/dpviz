...
> ### 📣 If you want more than read-only
>
> **Call Flow Studio** is the commercial successor to dpviz I've been building over the past year. For FreePBX, it's a full visual *editor* (not just a viewer) — palette, drag-create, undo/redo, multi-user, live extension / queue / agent state, simulated time ("what does this dial plan do at 2 AM on a holiday?"). It also adds **read-only visualization for VitalPBX**, with multi-tenant scoping and per-extension diversion rendering.
>
> **dpviz isn't going anywhere** — it stays free and read-only for FreePBX. CFS is what you reach for when you want to edit on the canvas, or when you're on VitalPBX.
>
